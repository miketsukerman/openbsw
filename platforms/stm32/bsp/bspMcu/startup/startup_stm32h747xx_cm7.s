 ...
 *
 * Startup for STM32H747xx (Cortex-M7 core): vector table and Reset_Handler,
 * based on the STMicroelectronics CMSIS device startup template for STM32H7.
 * Vector table layout per reference manual RM0399.
 *
 * The vector table may be linked at a non-zero flash offset (e.g. behind the
 * Arduino Portenta H7 bootloader at 0x08040000), so Reset_Handler programs
 * SCB->VTOR with the linked table address before anything else.
 *
...
/* Point VTOR at our vector table - the application may be linked at an
   offset behind a bootloader, so the reset default of 0x08000000 is wrong. */
  ldr r0, =0xE000ED08
  ldr r1, =g_pfnVectors
  str r1, [r0]
  dsb
...
/******************************************************************************
 * Vector table for STM32H747xx (150 external interrupts, RM0399 Table 143)
 ******************************************************************************/
