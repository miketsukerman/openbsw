...
.. _portenta_h7_main_entry:

main File
=========

The ``main.cpp`` module is the entry point of the Portenta H7 reference application.
``SystemInit()``, called from ``Reset_Handler``, configures the PLL to 480 MHz, switches
on the green LED (PK6, active-low), enables the GPIO port clocks needed during
bring-up, and sets up early USART1 TX output for use before the BSP UART
driver is initialized.

``main()`` constructs the safety supervisor, initializes the static BSP (system timer
and UART terminal, see :ref:`portenta_h7_StaticBsp`) and calls ``app_main()``, which
